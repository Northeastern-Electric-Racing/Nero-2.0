...
Rectangle {
    ...
    function fontSize(value, tileSize) {
        ...
        if (value >= 4096) return base * 0.55
        if (value >= 1024) return base * 0.64
        if (value >= 128) return base * 0.82
        return base
    }
    ...
    function slideLineWithMeta(line, sourceIndices) {
        ...
        while (k < compact.length) {
            if (k + 1 < compact.length && compact[k] === compact[k + 1]) {
                ...
                sources.push({ primary: compactSrc[k], consumed: compactSrc[k + 1] })
                mergeScore += compact[k] * 2
                k += 2
            } else {
                result.push(compact[k])
                sources.push({ primary: compactSrc[k], consumed: null })
                k++
            }
        }
        ...
        return { result: result, sources: sources, mergeScore: mergeScore }
    }
    ...
    function move(direction) {
        ...
        for (var i = 0; i < 4; i++) {
            ...
            for (var j = 0; j < 4; j++) {
                switch (direction) {
                case 0: line.push(board[idx(j, i)]); srcIndices.push(idx(j, i)); break
                case 1: line.push(board[idx(i, 3 - j)]); srcIndices.push(idx(i, 3 - j)); break
                case 2: line.push(board[idx(3 - j, i)]); srcIndices.push(idx(3 - j, i)); break
                case 3: line.push(board[idx(i, j)]); srcIndices.push(idx(i, j)); break
                }
            }
            ...
            for (j = 0; j < 4; j++) {
                var destIdx
                switch (direction) {
                case 0: destIdx = idx(j, i); break
                case 1: destIdx = idx(i, 3 - j); break
                case 2: destIdx = idx(3 - j, i); break
                case 3: destIdx = idx(i, j); break
                }
        ...
        var changed = false
        for (i = 0; i < 16; i++) {
            if (board[i] !== oldBoard[i]) { changed = true; break }
        }
        if (!changed) return
        ...
        for (i = 0; i < allSources.length; i++) {
            ...
            if (src.consumed !== null) {
                ...
                toRelease.push({ slot: consumedSlot, value: src.value, destSlot: primarySlot })
            }
        }
    ...
    function canMove() {
        ...
        for (var row = 0; row < 4; row++) {
            for (var col = 0; col < 4; col++) {
                ...
                if (col < 3 && v === board[idx(row, col + 1)]) return true
                if (row < 3 && v === board[idx(row + 1, col)]) return true
            }
        }
        return false
    }
        ...
        Item {
            ...
            Repeater {
                ...
                Rectangle {
                    ...
                    SequentialAnimation {
                        id: popAnim
                        NumberAnimation { target: tile; property: "scale"; from: 0; to: 1.2; duration: 100; easing.type: Easing.OutQuad }
                        NumberAnimation { target: tile; property: "scale"; to: 1.0; duration: 100; easing.type: Easing.InQuad }
                    }
                    ...
                    function spawn() { scale = 0; spawnAnim.start() }
    ...
    Connections {
        ...
        function onMoveMade(direction) { handleMove(direction) }
        function onRestartRequested() {
            if (showWin) dismissWin()
            else startGame()
        }
    }
}
