...
void RaspberryModel::connectToMQTT() {
    ...
    // FIX: Pass unique client IDs to avoid broker kicking duplicate connections
    MqttClient *client_1 =
        new MqttClient(nullptr, client1_port, client_1_topics, mqttHost,
                       "nero-data-subscriber");
    ...
    // FIX: Pass unique client ID for client_2
    MqttClient *client_2 =
        new MqttClient(nullptr, client2_port, client_2_topics, mqttHost,
                       "nero-button-subscriber");
...
void RaspberryModel::receiveServerData(const serverdata::v2::ServerData data, const QString topic) {
    QMetaObject::invokeMethod(this, "handleServerData", Qt::QueuedConnection,
                              Q_ARG(serverdata::v2::ServerData, data),
                              Q_ARG(QString, topic));
}

void RaspberryModel::handleServerData(const serverdata::v2::ServerData data, const QString topic) {
    if (data.values().isEmpty()) {
        qWarning() << "Received empty values for topic:" << topic << "- ignoring";
        return;
    }
    currentData[topic] = DataInfo(topic, data.unit(), data.values());
    emit onCurrentDataChange();
}
