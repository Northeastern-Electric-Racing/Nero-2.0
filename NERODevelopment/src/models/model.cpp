...
void Model::addPinnedData(QString id) {
    ...
    pinnedData.insert(id, DebugPlotValue(value.topic, value.unit, QList<float>(value.value)));
}
...
QList<DebugTableRowValue> Model::getDebugTableValues() {
  QList<DebugTableRowValue> table;
    for (DataInfo dataInfo : this->currentData.values()) {
        ...
        DebugTableRowValue row = {name,
        (std::round(value * 1000) / 1000), units};
...
std::optional<float> Model::getById(QString id) { return this->currentData[id].value; }
