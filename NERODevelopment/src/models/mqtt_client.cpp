...
MqttClient::MqttClient(QObject *parent, int port, QList<QString> topics,
                       QString hostname, const QString &clientId)
    : QObject(parent) {
    ...
    // Set unique client ID - critical to avoid broker kicking duplicate IDs
    if (clientId.isEmpty()) {
        m_client->setClientId("nero-" + QUuid::createUuid().toString(QUuid::WithoutBraces).left(8));
    } else {
...
void MqttClient::receiveMessage(const QByteArray &message,
    ...
    // qDebug() << "Raw message on" << topic.name() << "size:" << message.size() << "hex:" << message.toHex();

    serverdata::v2::ServerData serverData;
    bool success = m_serializer.deserialize(&serverData, message);
    if (success) {
        qDebug() << "Deserialized values:" << serverData.values();
...
void MqttClient::publishRaw(const QString topic, const float value) {
    serverdata::v2::ServerData serverData;
    serverData.setUnit("");
    serverData.setValues({value});
