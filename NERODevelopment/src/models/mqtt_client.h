...
public:
    // Updated constructor to accept optional hostname and clientId
...
public slots:
   ...
   */
    ...
    /**
   * @brief publishRaw, Publishes a protobuf message to the exact topic
   * (without NERO/ prefix). Used by ButtonSocketReceiver.
   * @param topic, the exact MQTT topic to publish to
   * @param value, the float value to send
   */
    void publishRaw(const QString topic, const float value);
