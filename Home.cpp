...
void Home::updateStatus(bool status) {
  ...
  statusValue->setStyleSheet(
      status ? "color: green; font-size: 100pt; font-weight: bold"
             : "color: red; font-size: 100pt; font-weight: bold");
}
...
void Home::updatePackTemp(int packTemp) {
  QString packTempText =
      (packTemp != -1) ? (QString::number(packTemp) + "°") : "N/A";
  packTempValue->setText(packTempText);
}

void Home::updateMotorTemp(int motorTemp) {
  QString motorTempText =
      (motorTemp != -1) ? (QString::number(motorTemp) + "°") : "N/A";
  motorTempValue->setText(motorTempText);
}

void Home::updateStateCharge(int stateCharge) {
  QString stateChargeText =
      (stateCharge != -1) ? (QString::number(stateCharge) + "%") : "N/A";
